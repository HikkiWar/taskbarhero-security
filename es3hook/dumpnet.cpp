...
#undef GP
    ...
    for (size_t a = 0; a < nasm; a++) {
        ...
        for (size_t c = 0; c < nc; c++) {
            ...
            if (!cn || !ns) continue;
            // сетевой слой: SDK бэкенда и транспорт Unity/.NET
            if (!strstr(ns, "BackEnd") && !strstr(ns, "Backnd")
                && !strstr(ns, "UnityEngine.Networking")
                && !strstr(ns, "System.Net")
                && !strstr(ns, "WebSocket")) continue;
