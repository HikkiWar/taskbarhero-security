...
echo [*] SDK: %SDK_VER%
echo [*] Building es3hook_v3.dll and es3hook_v4.dll...
...
%CLEXE% /nologo /O2 /EHsc /MD /LD ^
    ...
    es3hook_v4.cpp ^
    /link ^
    /DLL /OUT:es3hook_v4.dll ^
    /LIBPATH:"%MSVC_LIB%" ^
    /LIBPATH:"%SDK_LIB%\%SDK_VER%\um\x64" ^
    /LIBPATH:"%SDK_LIB%\%SDK_VER%\ucrt\x64" ^
    kernel32.lib psapi.lib ucrt.lib vcruntime.lib msvcrt.lib

if errorlevel 1 ( echo [-] v4 FAILED & exit /b 1 )
echo [+] es3hook_v4.dll OK
