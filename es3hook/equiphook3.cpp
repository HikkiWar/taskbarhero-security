...
static DWORD WINAPI Worker(LPVOID)
{
    ...
    g_log = fopen("C:\\Users\\Gigabyte\\Desktop\\THB\\es3hook\\equip_hook3.log", "w");
    if (!g_log) return 0;
    ...
    for (size_t a = 0; a < nasm; a++) {
        ...
        for (size_t c = 0; c < ncls; c++) {
            ...
            while (true) {
                ...
                if (!mn) continue;
                // Метод экипировки называется не "Equip": в метаданных есть только
                // тип ETryEquipGearResult. Поэтому идём от классов, отвечающих
                // за перемещение предметов и слоты.
                static const char* CLS[] = {
                    "ItemMoveController", "ItemSlotCache", "GearSlot", "ItemSlot",
                    "UsableItemSlotCache", "ItemFunction", NULL };
                bool clsHit = false;
                for (int q = 0; CLS[q]; q++)
                    if (strstr(cname, CLS[q])) { clsHit = true; break; }
                static const char* MTH[] = {
                    "Gear", "Move", "Swap", "Wear", "Attach", "SetItem", "Change", NULL };
                bool mthHit = false;
                for (int q = 0; MTH[q]; q++)
                    if (strstr(mn, MTH[q])) { mthHit = true; break; }
                bool tbh = cns && strstr(cns, "TaskbarHero");
                if (!(clsHit || (tbh && mthHit))) continue;
                ...
                found++;
                (void)fn;   // разведка: пока только перечисляем
            }
        }
    }
