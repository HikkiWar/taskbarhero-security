...
static DWORD WINAPI Worker(LPVOID)
{
    ...
    g_log = fopen("C:\\Users\\Gigabyte\\Desktop\\THB\\es3hook\\equip_hook4.log", "w");
    if (!g_log) return 0;
    ...
    for (size_t a = 0; a < nasm; a++) {
        ...
        for (size_t c = 0; c < ncls; c++) {
            ...
            while (true) {
                ...
                if (!mn) continue;
                // Имена методов обфусцированы (lif, lig, lji...), поэтому
                // цепляемся за то, что переименовать нельзя: реализации
                // Unity-интерфейсов (OnPointerDown/Up) и уцелевший тип MoveRequest.
                bool want = false;
                if ((strstr(cname, "GearSlot") || strstr(cname, "ItemSlot")) &&
                    (!strcmp(mn, "OnPointerDown") || !strcmp(mn, "OnPointerUp")))
                    want = true;
                if (strstr(cname, "MoveRequest"))
                    want = true;
                if (!want) continue;
